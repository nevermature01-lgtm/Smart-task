...
export default function CreateTeamPage() {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-6 modal-overlay">
      <button 
        onClick={() => router.back()}
        className="absolute top-14 left-6 w-12 h-12 flex items-center justify-center rounded-full glass-panel text-white active:scale-90 transition-all z-[60] shadow-lg border-white/20">
        ...
        <div className="relative z-10 space-y-8">
          ...
          <div className="space-y-4">
            <div className="space-y-2">
              ...
              <input className="w-full h-14 px-5 rounded-2xl input-glass text-white placeholder:text-white/40 focus:outline-none focus:ring-2 focus:ring-white/30 transition-all font-bold" id="team-name" placeholder="e.g. Design Lab" type="text"/>
            </div>
          </div>
          <div className="pt-2 flex flex-col gap-4">
            <button className="w-full h-14 bg-white text-primary rounded-2xl font-bold text-lg active:scale-[0.98] transition-all shadow-xl shadow-black/20">
                    Create
                </button>
            <button 
              onClick={() => router.back()}
              className="w-full py-2 text-white/60 font-bold text-sm active:opacity-70 transition-opacity">
                    Cancel
                </button>
          </div>
        </div>
